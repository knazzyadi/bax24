...
import { 
  ...
  ArrowUpRight, TrendingUp, Wrench, ShieldCheck, Zap, Loader2
} from "lucide-react";
  ...
  return (
    ...
    >
      ...
      <header className="space-y-1">
        <h1 className="text-3xl font-bold tracking-tight text-foreground">{t('header.title')}</h1>
        <p className="text-muted-foreground font-medium">{t('header.welcome')}</p>
      </header>
...
function QuickActionLink({ href, title, icon: Icon, description, variant, isRTL }: QuickActionProps & { isRTL: boolean }) {
  const variants = {
