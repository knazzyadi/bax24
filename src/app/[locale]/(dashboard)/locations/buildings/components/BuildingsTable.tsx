// src/app/[locale]/(dashboard)/locations/buildings/components/BuildingsTable.tsx

'use client';

import { useState } from 'react';
import { Building, Pencil, Trash2, Search, ChevronUp, ChevronDown, MapPin } from 'lucide-react';
import { useTranslations } from 'next-intl';
import { cn } from '@/lib/utils';
import { Building as BuildingType, Branch } from '../types';
      ...
      <div className={glassCard}>
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
            {isRTL
              ? 'ابدأ بإضافة مبنى جديد لتنظيم المواقع'
              : 'Start by adding a new building to organize locations'}
          </p>
        </div>
      </div>
    );
            ...
            {isLoading ? (
              ...
              buildings.map((building, index) => (
                ...
                >
                  ...
                  <td className="p-3 font-medium text-slate-700 dark:text-slate-200">{building.name}</td>
                  <td className="p-3 text-slate-600 dark:text-slate-300">{building.nameEn || '—'}</td>
                  <td className="p-3 font-mono text-slate-600 dark:text-slate-300">{building.code}</td>
                  <td className="p-3 text-slate-600 dark:text-slate-300">{building.order}</td>
                  <td className="p-3 text-slate-600 dark:text-slate-300">{building.branchName || '—'}</td>
                  <td className="p-3">
