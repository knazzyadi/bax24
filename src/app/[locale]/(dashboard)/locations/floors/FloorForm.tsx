...
import { floorSchema, type FloorFormValues } from './floor.schema';
import { cn } from '@/lib/utils';

// ✅ تعديل glassCard: إضافة خلفية متباينة و p-6
const glassCard =
...
export function FloorForm({
  ...
  return (
    <div className={glassCard}>
      {/* ✅ زيادة المسافة بين الرأس والنموذج إلى mb-6 */}
      ...
      {/* ✅ زيادة التباعد بين الحقول إلى gap-5 */}
      <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-2 gap-5">
        <div className="space-y-1">
          ...
          {errors.buildingId && (
            <p className="text-sm text-rose-500 mt-1">{errors.buildingId.message}</p>
          )}
        </div>
