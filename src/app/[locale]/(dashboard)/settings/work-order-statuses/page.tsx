// src/app/[locale]/(dashboard)/settings/work-order-statuses/page.tsx
'use client';
...
import { Plus, Pencil, Trash2, X, Loader2, Circle, Sparkles } from 'lucide-react';
...
const COLOR_OPTIONS = [
  ...
  { value: '#22c55e', nameAr: 'أخضر (مكتمل)', nameEn: 'Green (Completed)' },
...
function WorkOrderStatusesPageContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(editingStatus ? t('updateSuccess') : t('createSuccess'));
      setShowForm(false);
      setEditingStatus(null);
      await fetchStatuses();
      router.refresh();
    } catch (err: any) {
      toast.error(err.message || (editingStatus ? t('updateError') : t('createError')));
    } finally {
      setSubmitting(false);
    }
  };
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder={isRtl ? 'مثال: قيد التنفيذ' : 'Example: In Progress'}
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-slate-600 dark:text-slate-300">
                          {getColorLabel(status.color || '#64748b')}
                        </span>
                      </div>
                    </TableCell>
                    <TableCell>
