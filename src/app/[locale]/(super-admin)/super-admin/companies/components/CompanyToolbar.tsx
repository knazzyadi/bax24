// src/app/[locale]/(super-admin)/super-admin/companies/components/CompanyToolbar.tsx
'use client';
...
export function CompanyToolbar({ onAdd, total }: CompanyToolbarProps) {
  return (
    <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100">إدارة الشركات</h1>
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">إدارة الشركات المسجلة في النظام</p>
          {typeof total === 'number' && (
            <p className="mt-2 text-sm text-slate-500 dark:text-slate-500">
              {total} {total === 1 ? 'شركة' : 'شركات'}
            </p>
          )}
        </div>
      </div>
